...
test("generation sheet enforces same-template bounded material selection and server capabilities",()=>{for(const contract of ["maxMaterialsPerTask","maxEvidenceBlocks","generationEnabled","updateTemplateKey","idempotencyKey","crypto.randomUUID","caps.create","capabilityEnvelope.provider?.enabled"])assert.match(renderers,new RegExp(contract.replace(/[.?]/g,match=>`\\${match}`)));for(const copy of ["已选择","与已选材料模板不同","暂无可用于生成的材料","正在创建生成任务…","关闭生成面板","更新生成当前未启用；材料、证据和已有提案仍可查看。"])assert.match(renderers,new RegExp(copy.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));assert.doesNotMatch(renderers,/evidenceCount\\s*>\\s*maxEvidence/);});

test("Phase 6 UI keeps review, merge, publish and rollback on canonical controlled routes",()=>{for(const contract of ["接受此项","驳回此项","编辑后接受","事务合并到草稿","受控发布","回滚到直接上一发布版本","/review/","/merge","/release","/publish","/rollback"])assert.match(renderers,new RegExp(contract.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));assert.match(renderers,/AI 无法执行这些动作/);assert.match(renderers,/提案不能直达发布/);assert.doesNotMatch(source,/innerHTML|insertAdjacentHTML|eval\(|new Function/);});
