...
import { insertOrIgnore, upsert } from "../db/sql-dialect.mjs";
...
export function createMaterialService(database, options = {}) {
  ...
  async function createManual(principal, projectId, input) {
    ...
    try { withTransaction(database,()=>{
      ...
      database.prepare("INSERT INTO material_generation_grants (project_id,material_id,enabled,granted_by,granted_at) VALUES (?,?,1,?,?)").run(projectId,id,principal.id,at);
  ...
  function setGeneration(principal,projectId,materialId,input){const permission=access(principal,projectId);if(!permission.admin)throw new MaterialServiceError(404,"MATERIAL_NOT_FOUND","材料不存在或你无权访问");material(projectId,materialId);const enabled=input.enabled===true;withTransaction(database,()=>{upsert(database,"material_generation_grants",["project_id","material_id","enabled","granted_by","granted_at"],[projectId,materialId,enabled?1:0,enabled?principal.id:null,enabled?timestamp(now()):null],["project_id","material_id"]);audit(principal,"material.generation_access_changed",projectId,materialId,{enabled});});return detail(principal,projectId,materialId);}
