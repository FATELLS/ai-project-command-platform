  ...
  async function createJob(principal, projectId, input) {
    ...
    setImmediate(() => {
      ...
      generation.processJob(projectId, job.id)
        ...
        .catch(error => { console.error(`[gen] ${new Date().toISOString()} job=${job.id} processing FAILED in ${Date.now() - asyncStart}ms | code=${error?.code ?? "INTERNAL_ERROR"}`); });
    });
    return { task: job };
  }
        ...
        if (options.syncProcess) {
          ...
          setImmediate(() => {
            ...
            generation.processJob(projectId, job.id)
              ...
              .catch(error => { console.error(`[gen] ${new Date().toISOString()} job=${job.id} batch processing FAILED in ${Date.now() - batchStart}ms | code=${error?.code ?? "INTERNAL_ERROR"}`); });
          });
          results.push(job);
        }
      }
