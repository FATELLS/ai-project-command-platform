...
export function createReviewService(database,options={}) {
  ...
  function lockedContext(projectId,proposal){const graph=currentBase(projectId,proposal),job=proposals.getJob(projectId,proposal.jobId);if(!job)throw reviewError("GENERATION_JOB_NOT_FOUND","生成任务不存在",409);return{projectId,baseVersionId:proposal.baseVersionId,templateId:proposal.template.id,templateVersion:proposal.template.version,materials:job.materials,evidence:job.evidence,published:{units:graph.units,stages:graph.stages,tasks:graph.tasks,risks:graph.risks,metrics:graph.metrics,outcomes:graph.closures}};}
  ...
  function setDecision(principal,projectId,proposalId,changeId,input){const access=permission(principal,projectId);if(!access.review)throw reviewError("CHANGE_PROPOSAL_NOT_FOUND","更新建议不存在或你无权访问",404);const proposal=getProposal(projectId,proposalId);if(proposal.status!=="pending")throw reviewError("PROPOSAL_REVIEW_CLOSED","建议审核已结束",409);const change=proposal.changes.find(item=>item.changeId===changeId);if(!change)throw reviewError("PROPOSAL_CHANGE_NOT_FOUND","变更项不存在或你无权访问",404);const decision=input.decision;if(!["accepted","rejected"].includes(decision))throw reviewError("INVALID_REVIEW_DECISION","审核决定无效",400);let patch=null;if(decision==="accepted"&&input.patch!==undefined){if(!plainObject(input.patch))throw reviewError("INVALID_REVIEW_PATCH","编辑字段必须是有效的变更项",400);patch=input.patch;}const note=String(input.note??"").trim();if(note.length>500)throw reviewError("INVALID_REVIEW_NOTE","审核说明不能超过 500 字",400);const changes=proposal.changes.map(item=>item.changeId===changeId?{...item,patch:patch??item.patch}:item);revalidate(projectId,proposal,changes);return withTransaction(database,()=>{const at=clock();database.prepare(`UPDATE proposal_review_items SET decision=?,edited_patch_json=?,note=?,reviewed_by=?,reviewed_at=?,updated_at=? WHERE project_id=? AND proposal_id=? AND change_id=?`).run(decision,patch?JSON.stringify(patch):null,note,principal.id,at,at,projectId,proposalId,changeId);const states=rows(projectId,proposalId);if(states.every(item=>item.decision==="rejected"))database.prepare("UPDATE change_proposals SET status='rejected',updated_at=? WHERE project_id=? AND id=?").run(at,projectId,proposalId);audit(principal,`proposal.${decision}`,projectId,"change_proposal_item",`${proposalId}:${changeId}`,{edited:Boolean(patch),note:Boolean(note)});return getReview(principal,projectId,proposalId);});}
  ...
  function previewModule(principal, projectId, proposalId, moduleType) {
    ...
    // 确定使用哪些 changes：如果有审核决定，只用 accepted 的；否则用全部（待审核状态展示完整预览）
    const decisions = rows(projectId, proposalId);
    const byId = new Map(decisions.map(item => [item.changeId, item]));
    const hasDecisions = decisions.length > 0 && decisions.some(item => item.decision === "accepted" || item.decision === "rejected");
    const applicableChanges = hasDecisions
      ? proposal.changes.filter(item => byId.get(item.changeId)?.decision === "accepted").map(item => ({ ...item, patch: byId.get(item.changeId)?.editedPatch ?? item.patch }))
      : proposal.changes;
    ...
    return {
      ...
      pendingCount: hasDecisions ? 0 : proposal.changes.length,
      acceptedCount: hasDecisions ? applicableChanges.length : proposal.changes.length
    };
  }

  return Object.freeze({getReview,setDecision,acceptModule,merge,previewModule,permission});
}
