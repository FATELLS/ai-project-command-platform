function decode(row) {
  return { id: row.externalId, materialId: row.materialId, extractionVersion: row.extractionVersion, ordinal: row.ordinal, kind: row.kind, location: JSON.parse(row.locationJson), text: row.text, summary: row.summary };
}
...
export function createEvidenceService(database, options = {}) {
  ...
  function search({ projectId, query, audience = "project_member", limit = maxResults }) {
    ...
    let rows;
    if ([...value].length < 3) {
      rows = database.prepare(`SELECT ${columns} FROM evidence_blocks b ${shared} AND (b.text LIKE ? ESCAPE '\\' OR b.summary LIKE ? ESCAPE '\\') ORDER BY b.material_id, b.ordinal LIMIT ?`)
        .all(projectId, access(audience), `%${value.replace(/[\\%_]/g, "\\$&")}%`, `%${value.replace(/[\\%_]/g, "\\$&")}%`, capped);
    } else {
      const phrase = `"${value.replaceAll('"', '""')}"`;
      rows = database.prepare(`SELECT ${columns}, bm25(evidence_fts) AS rank FROM evidence_fts JOIN evidence_blocks b ON b.id = evidence_fts.rowid ${shared} AND evidence_fts MATCH ? ORDER BY rank, b.material_id, b.ordinal LIMIT ?`)
