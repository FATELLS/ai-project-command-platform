...
export function createOpenAiCompatibleProvider(config, options = {}) {
  ...
  return Object.freeze({ configured: true, safeLabel: providerLabel, async generate(request, { signal } = {}) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      const timeoutHandle = setTimeout(() => deadline.abort(), configuredTimeoutMs);
      ...
      try {
        ...
        try {
          ...
          const code = error?.name === "AbortError" ? "AI_PROVIDER_TIMEOUT" : "AI_PROVIDER_NETWORK_ERROR";
          console.error(`[provider] ${new Date().toISOString()} FETCH FAILED after ${elapsed}ms | code=${code}`);
          if (signal?.aborted) throw safeProviderError("AI_PROVIDER_ABORTED"); if (attempt === 0 && error?.name !== "AbortError") { await sleep(1); continue; } throw safeProviderError(code);
        }
