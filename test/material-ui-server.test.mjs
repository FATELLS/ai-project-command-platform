...
test("materials use canonical project-scoped ledger, detail, evidence and chat APIs", () => {
  for (const contract of [
    "/materials", "/capabilities", "/manual", "/upload", "/update-template", "/retry", "/evidence"
...
test("ledger exposes server-authoritative quotas, gates, persistent states and role-gated actions", () => {
  ...
  for (const flag of ["caps.upload", "caps.manual", "caps.retry", "caps.selectUpdateTemplate"]) assert.match(renderers, new RegExp(flag.replace(".", "\\.")));
  for (const code of ["duplicate_material", "project_material_limit", "project_capacity_limit", "upload_concurrency_limited"]) assert.match(renderers, new RegExp(code));
...
test("detail and Q&A preserve exact evidence locators, citations and honest refusal states", () => {
  for (const copy of [
    "返回项目材料", "选择内容位置", "内容片段", "未提供精确区域", "查看提取文本",
    "第 ", "张幻灯片", "工作表", "单元格", "材料不存在或你无权访问", "更新模板已记录",
    "只读取当前项目已发布状态和已授权材料；回答不会修改项目数据。", "引用来源",
    "现有资料不足以回答这个问题。", "暂无可用于问答的授权材料", "发送问题",
    "正在查找依据…", "暂时无法完成项目问答。已保留你的问题，请稍后重试。"
  ]) assert.match(renderers, new RegExp(copy.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  assert.match(renderers, /location\.type === "sheet-cell" && location\.cell/);
