...
function setup() {
  ...
  // 同步写入 project_cards（统一卡片表）
  db.prepare("INSERT OR IGNORE INTO project_cards (version_id,external_id,element_type,position,title,owner,state,objective,start_date,end_date,progress,health,unit_id,parent_id,depends_on,card_attrs,created_at,updated_at) VALUES (?,'unit-a','unit',0,'Team A','','active','目标','','',NULL,'','','','[]',?,?,?)").run(published, unitData, at, at);
  db.prepare("INSERT OR IGNORE INTO project_cards (version_id,external_id,element_type,position,title,owner,state,objective,start_date,end_date,progress,health,unit_id,parent_id,depends_on,card_attrs,created_at,updated_at) VALUES (?,'task-001','task',1,'开放任务','负责人','进行中','','2026-07-01','2026-07-31',30,'','unit-a',NULL,'[]',?,?,?)").run(published, taskData, at, at);
