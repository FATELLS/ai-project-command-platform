...
最后更新：2026-08-02（PMBOK 提示词改造 + 配置加载修复 + 生成管道可观测性强化 + AGENTS.md 留痕规则）

- 项目状态：`active`
- 当前阶段：全项目 SYSTEM-SPEC、Architecture、Traceability 和八个系统模块 ADR/DESIGN 已建立；Phase 11 为 `implementation in progress`，第一实施切片已验证
- 当前结果：Phase 1–10 全部实施完毕；Phase 11 第一切片、多文档到路线图 UI 复跑、源码后台生命周期、统一材料模板、异常材料闭环、以本次材料为起点的独立项目更新四步流程、正式路线图 renderer 复用，以及路线图卡片受控编辑均已实现并验证；PMBOK 七元素端到端贯通（提示词→上下文→写入合并）；生成管道可观测性强化（全链路时间戳日志）
- 已实现平台功能：数据底座、Xugu 迁移、认证/成员、项目切换、九类固定模块、材料证据问答、结构化提案、审核合并、发布回滚、材料 readiness、作战单元生命周期、运维诊断、自检中心、审计、备份恢复、项目导入导出、路线图五视图工作台（卡片泳道默认·双锚点·生命周期分带·分形作战模型）与交互式受控提案、PMBOK 七元素提取框架、生成管道全链路可观测性日志
- 平台版本：`0.9.6`（npm 已发布）
- Git HEAD：`15dd870`（已推送）→ 本次新增未提交修改（生成管道可观测性 + AGENTS.md 留痕规则）
- Node 版本要求：`>=20.0 <23`（虚谷原生驱动 ABI 115）
- 验证基线：**194 项单元测试**（194 pass / 0 fail）+ 49 项主流程 E2E + 9 项异常材料 E2E
- 虚谷 migration：8 文件全量验证通过，111 语句执行，47 表创建成功
- 下一步：(1) UI 端验证 PMBOK 生成端到端流程；(2) 依据 `.planning/phases/11-workflow-first-ui/11-VERIFICATION.md` 补齐部分完成项
- 当前阻塞：GLM-5.2 大 prompt 处理延迟（简单 prompt 2 秒返回，平台完整 prompt 需要更长时间——可观测性日志已就位，下次可精确定位瓶颈阶段）

## 2026-07-29 项目更新四步主流程

- 通用 `/projects/:projectId/updates` 不再自动选择最近 proposal，固定从“本次材料”开始；所有总览、顶栏和旧兼容入口都遵守同一规则。
- 更新工作区按“本次材料 → 处理与生成 → 模拟路线图 → 审核与发布”展示步骤。上传或选择已处理材料后进入具体 generation task，生成完成后进入绑定 `proposalId` 的预览路由。
- 历史待处理更新只作为“继续上一次更新”次级入口；模拟路线图和审核页均绑定具体 proposal，不能以通用入口跳过材料阶段。
- 新增 WUI-25 与 D-049；全量验证通过 182 项后台/静态测试、49 项主 E2E、9 项异常 E2E，以及全部浏览器证据、Xugu 等价和来源只读检查。

## 2026-07-29 共享图标语言与卡片操作位

- 顶栏工具、模块滚动、路线图图例/状态、项目创建方式和卡片编辑统一使用 `public/modules/shared.js` 的安全线性 SVG 图标目录，不再以 Emoji、文本箭头或可见“编辑”文字充当工具图标。
- 路线图编辑入口改为卡片内侧右上角的弱化铅笔按钮，保留 `aria-label` 和 `title`；展开主卡按实际列宽定位，窄任务卡采用可收缩标题与纵向状态布局。
- 桌面卡片工具保持紧凑，移动端触控区域仍为 44×44px；删除与发布等高风险操作继续显示文字。
- 新增 WUI-24 与 D-048；全量验证通过 181 项后台/静态测试、48 项主 E2E、9 项异常 E2E，以及全部浏览器证据、Xugu 等价和来源只读检查。

## 2026-07-29 项目资料与项目更新拆分

- 项目资料只保留战果档案与项目材料，负责材料输入、处理和生成任务；项目更新改为独立 `/projects/:projectId/updates` 流程，不占用第七个内容模块。
- 项目更新页只渲染一张“更新后的项目路线图”，直接调用正式路线图 `renderRoadmapSwimlane`；旧专用预览 DOM 和 CSS 已删除。
- 更新路线图仅高亮并允许编辑本次 added/modified 卡片，现有发布节点降亮只读；未排期节点由同一 renderer 的待排期区承接。
- 生成统计、批量生成、提案列表、生成任务列表和模板控制台均从项目更新页移除；材料生成成功后以“查看项目更新”进入该页。
- `npm run verify` 通过：180 项后台/静态测试、48 项主 E2E、9 项异常材料与节点预览 E2E，以及全部浏览器证据、Xugu 等价和来源只读检查。

## 2026-07-29 路线图卡片受控编辑

- 正式路线图的阶段和任务卡片均提供“编辑”；保存或删除只创建 `interaction` ChangeProposal，不直接改写 draft/published，关键字段与删除要求选择当前项目材料证据。
- 编辑器覆盖标题、描述、日期、状态、负责人、进度、健康度、依赖、交付物、风险、验收标准等卡片属性；数组字段支持逐项增删改。
- AI 节点预览只允许编辑本次 proposal 新增或修改的卡片；既有发布节点保留为定位上下文，使用降亮样式和“当前节点 · 只读”标签。
- 两处删除均必须精确输入“确认删除”。正式路线图删除进入受控审核；节点预览删除只驳回并移出本次候选卡片。
- 全量验证：180 项后台/静态测试、48 项主 E2E、9 项隔离异常与节点预览 E2E，全部浏览器证据通过。

## 2026-07-29 AI 节点预览与路线图投影

- 项目更新独立提供“AI 生成项目节点预览”；普通界面不再以提案 UUID 或“更新建议”为主心智。
- 节点卡片下默认复刻路线图结构，待决定/已接受项进入只读投影，已驳回项退出，编辑后接受使用审核值。
- AI 新增、修改和现有节点分色；任务按日期只归入一个阶段，AI 待排期卡片优先显示，历史待排期节点折叠。
- 人工材料归档与模板选择自动开启内部 generation grant，不增加任何用户可见授权步骤。
- 全量验证：175 项后台/静态测试、47 项主 E2E、9 项隔离异常与节点预览 E2E，全部浏览器证据通过。

## 2026-07-29 项目更新模板与异常材料

- 创建与项目更新共用六类 Markdown 模板目录，模板入口覆盖创建、总览、台账、材料录入、生成、节点预览和材料详情。
- 创建材料与更新材料的空文件、类型和内容签名门阀保持对称；创建文本还要求明确项目名称/Markdown 标题和项目目标。
- 异常 UI 套件覆盖空、伪装、未知、重复、缺少创建要素和阶段用途不匹配；失败不产生部分项目、材料或生成任务。
- 合法模板回归确认完整 Markdown 标题和目标可进入项目骨架确认。
- 当时验证基线：173 项后台/静态测试、47 项主 E2E、8 项隔离异常 E2E；最新基线见上方节点预览记录。

## 2026-07-29 平台后台生命周期与项目更新入口

- 新增 `npm run start:background/status/stop/restart`，后台实例通过 PID、日志和 `/health` 管理；`npm start` 继续作为前台开发模式。
- SIGTERM 关闭顺序固定为停止 HTTP 接入、停止材料 worker、等待请求收尾、释放平台数据库连接；20 秒未结束时不自动强杀。
- 平台停止命令不含 Docker/虚谷数据库操作，虚谷容器继续独立运行。
- 项目总览主操作从“上传项目材料”改为“项目更新”；该历史入口随后由 D-047 收敛为独立 `/updates` 流程，材料上传继续留在项目资料。
- 当时验证通过 171 项后台测试与 46 项 Playwright E2E；最新基线见本文件顶部。
...
- 项目健康二级入口为风险台账、效果指标；项目资料二级入口只保留战果档案与项目材料。AI 节点预览和审核发布由独立项目更新流程承接。
