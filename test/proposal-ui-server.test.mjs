...
test("fixed renderer exposes locked tasks, validated proposals, citations and honest usage",()=>{for(const copy of ["作战更新建议","项目更新建议","生成作战更新建议","生成项目更新建议","AI 只生成带来源的更新建议；不会修改项目草稿或发布版本。","等待生成资源","锁定并整理材料内容","执行服务端校验","更新建议已生成","发布基准已变化","服务端校验结果","变更字段","引用内容","未配置单价，仅记录 Token","尚未应用到草稿，也未发布"])assert.match(renderers,new RegExp(copy.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));assert.match(renderers,/safeText|text:/);assert.doesNotMatch(source,/innerHTML|insertAdjacentHTML|eval\(|new Function/);});

test("generation sheet enforces same-template bounded material selection and server capabilities",()=>{for(const contract of ["maxMaterialsPerTask","maxEvidenceBlocks","generationEnabled","updateTemplateKey","idempotencyKey","crypto.randomUUID","caps.create","capabilityEnvelope.provider?.enabled"])assert.match(renderers,new RegExp(contract.replace(/[.?]/g,match=>`\\${match}`)));for(const copy of ["已选择","与已选材料模板不同","暂无可用于生成的材料","正在创建生成任务…","关闭生成面板","更新生成当前未启用；材料和已有建议仍可查看。"])assert.match(renderers,new RegExp(copy.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));assert.doesNotMatch(renderers,/evidenceCount\\s*>\\s*maxEvidence/);});

test("Phase 6 UI keeps review, merge, publish and rollback on canonical controlled routes",()=>{for(const contract of ["接受此项","驳回此项","编辑后接受","应用到草稿","受控发布","回滚到直接上一发布版本","/review/","/merge","/release","/publish","/rollback"])assert.match(renderers,new RegExp(contract.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));assert.match(renderers,/AI 无法执行这些动作/);assert.match(renderers,/提案不能直达发布/);assert.doesNotMatch(source,/innerHTML|insertAdjacentHTML|eval\(|new Function/);});
