...
export function createProposalRepository(database, options = {}) {
  const now = options.now ?? Date.now;
  const timestamp = () => new Date(now()).toISOString();

  function createJob(input) {
    return withTransaction(database, () => {
      const existing = database.prepare("SELECT id FROM generation_jobs WHERE project_id=? AND created_by=? AND idempotency_key=?").get(input.projectId, input.createdBy, input.idempotencyKey);
      if (existing) return getJob(input.projectId, existing.id);
      const id = input.id ?? randomUUID(); const at = timestamp();
      database.prepare(`INSERT INTO generation_jobs (id,project_id,base_version_id,template_id,template_version,schema_version,idempotency_key,request_hash,created_by,retry_of_job_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`)
        .run(id,input.projectId,input.baseVersionId,input.templateId,input.templateVersion,input.schemaVersion,input.idempotencyKey,input.requestHash,input.createdBy,input.retryOfJobId??null,at,at);
      const material = database.prepare("INSERT INTO generation_job_materials (project_id,job_id,material_id,extraction_version,position,readiness_json) VALUES (?,?,?,?,?,?)");
      input.materials.forEach((item,index)=>material.run(input.projectId,id,item.id,item.extractionVersion,index,JSON.stringify(item.readiness??{})));
      const evidence = database.prepare("INSERT INTO generation_job_evidence (project_id,job_id,evidence_external_id,material_id,extraction_version,content_hash,position) VALUES (?,?,?,?,?,?,?)");
      input.evidence.forEach((item,index)=>evidence.run(input.projectId,id,item.evidenceId,item.materialId,item.extractionVersion,item.contentHash,index));
      return getJob(input.projectId,id);
    });
  }

  function getJob(projectId,id) {
    const row=database.prepare(`SELECT j.*,v.version_label AS baseVersionLabel,u.display_name AS createdByName FROM generation_jobs j JOIN project_versions v ON v.id=j.base_version_id JOIN users u ON u.id=j.created_by WHERE j.project_id=? AND j.id=?`).get(projectId,id);
    if(!row)return undefined;
    const materials=database.prepare(`SELECT l.material_id AS id,l.extraction_version AS extractionVersion,l.position,l.readiness_json AS readinessJson,m.display_name AS name FROM generation_job_materials l JOIN project_materials m ON m.project_id=l.project_id AND m.id=l.material_id WHERE l.project_id=? AND l.job_id=? ORDER BY l.position`).all(projectId,id).map(item=>({...item,readiness:parse(item.readinessJson,{})}));
    const evidence=database.prepare(`SELECT evidence_external_id AS evidenceId,material_id AS materialId,extraction_version AS extractionVersion,content_hash AS contentHash,position FROM generation_job_evidence WHERE project_id=? AND job_id=? ORDER BY position`).all(projectId,id);
    const attempts=database.prepare(`SELECT id,attempt_number AS attemptNumber,kind,outcome,provider_label AS providerLabel,input_tokens AS inputTokens,output_tokens AS outputTokens,latency_ms AS latencyMs,currency,price_version AS priceVersion,cost_micros AS costMicros,cost_status AS costStatus,result_code AS resultCode,created_at AS createdAt,finished_at AS finishedAt FROM generation_attempts WHERE project_id=? AND job_id=? ORDER BY attempt_number`).all(projectId,id);
    return {id:row.id,projectId:row.project_id,baseVersionId:row.base_version_id,baseVersionLabel:row.baseVersionLabel,template:{id:row.template_id,version:row.template_version},schemaVersion:row.schema_version,state:row.state,attemptsCount:row.attempts,errorCode:row.error_code,validation:parse(row.validation_json,{}),proposalId:row.proposal_id,retryOfJobId:row.retry_of_job_id,createdBy:row.createdByName,createdAt:row.created_at,updatedAt:row.updated_at,materials,evidence,attempts};
  }
